...
# --- TEST FUNCTION ---
def test_mycelial_seeds_system():
    """Test the mycelial seeds system."""
    print("\n" + "="*60)
    print("🌱 TESTING MYCELIAL SEEDS SYSTEM")
    print("="*60)
    
    try:
        # Mock brain structure with seeds
        mock_brain = {
            'mycelial_seeds': {
                'seed_001': {
                    'coordinates': (100, 100, 100),
                    'region': 'frontal_cortex',
                    'sub_region': 'prefrontal_cortex',
                    'hierarchical_name': 'frontal_cortex-prefrontal_cortex-seed-001',
                    'creation_time': datetime.now().isoformat(),
                    'energy_storage_capacity': 1000.0
                },
                'seed_002': {
                    'coordinates': (150, 150, 150),
                    'region': 'limbic_system',
                    'sub_region': 'anterior_cingulate',
                    'hierarchical_name': 'limbic_system-anterior_cingulate-seed-002',
                    'creation_time': datetime.now().isoformat(),
                    'energy_storage_capacity': 1000.0
                }
            }
        ...
        print("1. Creating mycelial seeds system...")
        seeds_system = create_mycelial_seeds_system(mock_brain)
        print(f"   ✅ System ID: {seeds_system.system_id[:8]}")
        
        print("\n2. Testing seed activation for entanglement...")
        result = seeds_system.activate_seed_for_entanglement('seed_001', 'seed_002', 'quantum_communication')
        print(f"   ✅ Entanglement activated: {result['success']}")
        
        print("\n3. Testing communication channel...")
        comm_result = seeds_system.activate_seed_for_communication('seed_002', (200, 200, 200))
        print(f"   ✅ Communication channel: {comm_result.get('communication_channel_id', 'None')[:8]}")
        
        print("\n4. Testing field modulation...")
        mod_result = seeds_system.activate_field_modulation('seed_001', 'healing', 'frontal_cortex', 1.5)
        print(f"   ✅ Field modulation: {mod_result['modulation_type']}")
        
        print("\n5. Getting system status...")
        status = seeds_system.get_system_status()
        print(f"   Active seeds: {status['operational_summary']['active_seeds']}")
        print(f"   Total energy used: {status['energy_summary']['total_energy_used']:.1f} SEU")
        
        print("\n✅ ALL MYCELIAL SEEDS TESTS PASSED!")
        return True
