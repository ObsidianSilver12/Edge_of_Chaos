...
import math
from shared.metrics_tracking import MetricsTracker
...
# create a unique frequency sound for each key from the base frequency of the key must be same quantum gateway signal pattern just with its own frequency
def create_unique_key_sound()

# create each platonic connection shape which can connect the model to those sephira through the key. must use correct element colour
def create_key_platonics_drawing()

# create a unique symbol for each sephira not to be repeated. use correct sephira colour for the symbol.
# make sure you are updating the dictionary that the symbol chosen is no longer available
def create_key_sephira_sigil()

# create an image for each sephira of their sigil and store under assets/sephira so we can use the image when desired
def create_key_sephira_image()

# create the sigil for the key no need to store as image make sure you are updating the dictionary that the sigil chosen is 
# no longer available
def create_key_sigil()
    
# draw the gateway - 2 circles must be able to fit the sephira images within the 2 circles between the outer and inner circle
# you will need to colour the first outer circle black and the inner circle white so that the sigils all show because 1 is white
def draw_the_gateway()

# complete the key glyph by adding the gateway, placing the sephira sigil (not image) between the outer and inner circle at same intervals 
# will need to rotate them so they are going around the circle, then inside the inner circle place the platonic shape and in the centre of the 
# platonic shape add the key sigil
def complete_key_glyph()
    
# encode the sephira data in the image glyphs using both exif for metadata and steganography for hiding the data
# encode the sound also, do we also add the mirrored encoding to this or just ensure the mirror encoding is done after this encoding?
def encode_key_sephira_data_in_image()

# capture relevant sensory data from the image glyphs created, from the sound created for each key and for 
# the other data being created here
def capture_key_sensory_data()
