# --- START OF FILE root_controller.py (CORRECTED V5.3) ---
"""
Root Controller (V5.3 - Corrected Architecture)
...
This ensures a clean separation of concerns and resolves all circular dependencies.
"""
...
try:
    ...
    try:
        ...
        raise ImportError("The module 'stage_1.soul_formation.soul_completion_controller' could not be found. Please ensure the file exists and the path is correct.") from exc
...
# """
# Root Controller (V5.0 - Soul Completion Integration)

# Integrates with the comprehensive soul completion controller while maintaining
# root controller structure for compatibility and extended functionality.

# This version delegates core soul development to the soul_completion_controller
# while providing root-level orchestration, configuration, and reporting.
# """
...
#     if "LOG_LEVEL" not in locals():
#         raise NameError("LOG_LEVEL not defined")
#     if "GRID_SIZE" not in locals():
#         raise NameError("GRID_SIZE not defined")
#     # ... (add all other necessary constant checks from your previous version) ...
#     if "DATA_DIR_BASE" not in locals():
#         raise NameError("DATA_DIR_BASE not defined")

#     logger_check = logging.getLogger("ConstantsCheckRoot")
#     logger_check.info(
#         f"RootController: Constants loaded. LOG_LEVEL={LOG_LEVEL}, GRID_SIZE={GRID_SIZE}"
#     )
...
#         logger.error(
#             f"RootController File logging setup failed: {log_err}", exc_info=True
#         )
#         logger.info(
#             "RootController Logging configured (Terminal: INFO, File: Disabled)."
#         )
# else:
...
#     from stage_1.soul_formation.harmonic_strengthening import (
#         perform_harmonic_strengthening,
#     )
#     from stage_1.soul_formation.identity_crystallization import (
#         perform_identity_crystallization,
#     )
...
#         process_sephirah_interaction,
#     )
#     from stage_1.soul_formation.spark_harmonization import perform_spark_harmonization
#     from stage_1.soul_spark.soul_spark import SoulSpark

#     logger.debug(
#         "RootController: Core components and stage functions imported successfully."
#     )
# except ImportError as e:
#     logger.critical(
#         "FATAL ROOT: Could not import core component or stage function: %s",
#         e,
#         exc_info=True,
#     )
#     sys.exit(1)
# except (NameError) as e:
#     logger.critical(
#         "FATAL ROOT: Error during core component/stage function imports: %s",
#         e,
#         exc_info=True,
#     )
#     sys.exit(1)
...
# VISUALIZATION_OUTPUT_DIR_ROOT = os.path.join(
#     DATA_DIR_BASE, "visuals", "simulation_run"
# )  # Unified visual output
# try:
#     os.makedirs(VISUALIZATION_OUTPUT_DIR_ROOT, exist_ok=True)  # Ensure directory exists
...
#     logger.critical(
#         "CRITICAL ROOT ERROR: Soul visualization module not found: %s. Visualizations are required. Aborting.", ie
#     )
#     sys.exit(1)
# except Exception as e:
#     logger.critical(
#         "CRITICAL ROOT ERROR: Error setting up visualization: %s. Aborting.",
#         e,
#         exc_info=True,
#     )
#     sys.exit(1)
...
# except ImportError:
#     logger.warning(
#         "RootController: Metrics tracking module not found. Using placeholder."
#     )
...
#         @staticmethod
#         def persist_metrics(*args, **kwargs):
#             pass

#     metrics = MetricsPlaceholder()  # type: ignore
# except Exception as e:  # Catch any error during metrics import
#     logger.critical("CRITICAL ROOT ERROR metrics import: %s", e, exc_info=True)
...
# except ImportError:
#     logger.warning(
#         "Mother resonance module not found for RootController. Birth will proceed without mother influence or require explicit profile."
#     )
...
#         "initial_state",
#         "final_state",
#         "guff_properties_used",
#         "imparted_aspect_strengths",
#         "aspects_touched_names",
#         "initial_state_changes",
#         "geometric_changes",
#         "local_entanglement_changes",
#         "element_details",
#         "cycle_details",
#         "components",
#         "missing_attributes",
...
#         "transfers",
#         "memory_retentions",
#         "layer_formation_changes",
#         "imparted_aspect_strengths_summary",
#         "detailed_metrics",
#         "guff_targets_used",
#         "initial_stability_su",
#         "initial_coherence_cu",
#         "initial_energy_seu",
#         "initial_pattern_coherence",
#         "initial_phi_resonance",
#         "initial_harmony",
#         "initial_toroidal_flow",
#         "peak_stability_su_during_stabilization",
#         "peak_coherence_cu_during_stabilization",
#         "step_metrics",
#     }
...
#     max_key_len = (
#         max(len(k.replace("_", " ").title()) for k in display_keys)
#         if display_keys
#         else 30
#     )
...
#         key_display = key
#         key_lower = key.lower()
#         if key_lower.endswith("_seu"):
#             unit = " SEU"
#             key_display = key[:-4]
#         elif key_lower.endswith("_su"):
#             unit = " SU"
#             key_display = key[:-3]
#         elif key_lower.endswith("_cu"):
#             unit = " CU"
#             key_display = key[:-3]
#         # ... (rest of the unit logic from your display_stage_metrics) ...
#         elif key_lower.endswith("_hz"):
#             unit = " Hz"
#             key_display = key[:-3]
#         elif key_lower.endswith("_pct"):
#             unit = "%"
#             key_display = key[:-4]
#         elif key_lower.endswith("_factor"):
#             key_display = key[:-7]
#         elif key_lower.endswith("_score"):
#             key_display = key[:-6]
#         elif key_lower.endswith("_level"):
#             key_display = key[:-6]
#         elif key_lower.endswith("_count"):
#             key_display = key[:-6]
#         elif key_lower.endswith("_ratio"):
#             key_display = key[:-6]
#         elif key_lower.endswith("_strength"):
#             key_display = key[:-9]
#         elif key_lower.endswith("_coherence"):
#             key_display = key[:-10]
#         elif key_lower.endswith("_resonance"):
#             key_display = key[:-10]
#         elif key_lower.endswith("_alignment"):
#             key_display = key[:-10]
#         elif key_lower.endswith("_integration"):
#             key_display = key[:-12]
...
#             formatted_val = f"<List ({len(value)} items)>"
#         elif isinstance(value, dict):
#             formatted_val = f"<Dict ({len(value)} keys)>"
#         else:
#             formatted_val = str(value)
...
# def run_simulation(
#     num_souls: int = 1,  # Changed to 1 as per your intent
...
#     if num_souls != 1:
#         logger.warning(
#             "RootController is configured to run for a single soul. num_souls > 1 will only process the first."
#         )
#         num_souls = 1  # Enforce single soul

#     logger.info(
#         "--- Starting Soul Development Simulation (V4.3.11 - Direct Full Soul Completion) ---"
#     )
#     logger.info(
#         f"Num Souls: {num_souls}, Sephirah Duration: {journey_duration_per_sephirah}"
#     )
#     overall_start_time = time.time()
...
#             creation_location_coords = field_controller._coords_to_int_tuple(
#                 creation_location
#             )
#             soul_spark = SoulSpark.create_from_field_emergence(
#                 field_controller, creation_location_coords
#             )
#             process_summary["soul_id"] = soul_spark.spark_id
#             logger.info(
#                 f"Soul Spark {soul_spark.spark_id} emerged at {creation_location_coords}."
#             )
#             emergence_metrics = {
#                 "success": True,
#                 **soul_spark.get_spark_metrics()["core"],
#             }
#             process_summary["stages_metrics"][current_stage_name] = emergence_metrics
#             if METRICS_AVAILABLE:
#                 metrics.record_metrics(
#                     f"root_{current_stage_name.lower().replace(' ', '_')}",
#                     emergence_metrics,
#                 )
...
#                     visualize_soul_state(
#                         soul_spark,
#                         "Spark_Emergence_Initial",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                 except Exception as vis_err:
#                     raise RuntimeError(
#                         f"Visualization error at {current_stage_name}: {vis_err}"
#                     ) from vis_err

#             # --- Stage 2: Spark Harmonization ---
#             current_stage_name = "Spark Harmonization"
#             logger.info(f"RootController Stage: {current_stage_name}...")
#             _, harm_metrics = perform_spark_harmonization(
#                 soul_spark,
#                 iterations=kwargs.get(
#                     "harmonization_iterations", HARMONIZATION_ITERATIONS
#                 ),
#             )
...
#             logger.info(
#                 f"RootController Stage: {current_stage_name} (including move)..."
#             )
...
#                     visualize_soul_state(
#                         soul_spark,
#                         "Pre_Guff_Strengthening",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                 except Exception as vis_err:
#                     raise RuntimeError(
#                         f"Visualization error at {current_stage_name} (Pre): {vis_err}"
#                     ) from vis_err
...
#             field_controller.release_soul_from_guff(
#                 soul_spark
#             )  # Release after strengthening
...
#                     visualize_soul_state(
#                         soul_spark,
#                         "Post_Guff_Strengthening",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                 except Exception as vis_err:
#                     raise RuntimeError(
#                         f"Visualization error at {current_stage_name} (Post): {vis_err}"
#                     ) from vis_err
...
#                 "malkuth",
#             ]
#             journey_overall_metrics: Dict[str, Any] = {
#                 "steps": {},
#                 "success": True,
#                 "total_duration": 0.0,
#             }
...
#                 if not sephirah_influencer or not isinstance(
#                     sephirah_influencer, SephirothField
#                 ):
#                     raise RuntimeError(
#                         f"SephirothField missing for '{sephirah_name_iter}'."
#                     )
#                 _, step_metrics = process_sephirah_interaction(
#                     soul_spark=soul_spark,
#                     sephirah_influencer=sephirah_influencer,
#                     field_controller=field_controller,
#                     duration=journey_duration_per_sephirah,
#                 )
#                 journey_overall_metrics["steps"][sephirah_name_iter] = step_metrics
#                 journey_overall_metrics["total_duration"] += (
...
#                 if VISUALIZATION_ENABLED and sephirah_name_iter in [
#                     "kether",
#                     "tiphareth",
#                     "malkuth",
#                 ]:
...
#                         visualize_soul_state(
#                             soul_spark,
#                             f"Journey_{sephirah_name_iter.capitalize()}",
#                             VISUALIZATION_OUTPUT_DIR_ROOT,
#                             show=show_visuals,
#                         )
#                     except Exception as vis_err:
#                         raise RuntimeError(
#                             f"Visualization error at Journey ({sephirah_name_iter}): {vis_err}"
#                         ) from vis_err
#             setattr(soul_spark, FLAG_SEPHIROTH_JOURNEY_COMPLETE, True)
#             setattr(soul_spark, FLAG_READY_FOR_ENTANGLEMENT, True)
#             process_summary["stages_metrics"][current_stage_name] = (
#                 journey_overall_metrics
#             )
#             if VISUALIZATION_ENABLED:
#                 try:
...
#                 raise RuntimeError("Kether field unavailable for Entanglement.")
#             if VISUALIZATION_ENABLED:
#                 try:
#                     development_states.append((soul_spark, "Pre_Creator_Entanglement"))
#                     visualize_soul_state(
#                         soul_spark,
#                         "Pre_Creator_Entanglement",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                 except Exception as vis_err:
...
#                 soul_spark=soul_spark,
#                 kether_field=kether_influencer,
#                 base_creator_potential=kwargs.get(
#                     "base_creator_potential", CREATOR_POTENTIAL_DEFAULT
#                 ),
#                 edge_of_chaos_target=kwargs.get(
...
#             setattr(
#                 soul_spark, FLAG_READY_FOR_HARMONIZATION, True
#             )  # Set by perform_creator_entanglement
#             if VISUALIZATION_ENABLED:
#                 try:
#                     development_states.append((soul_spark, "Post_Creator_Entanglement"))
#                     visualize_soul_state(
#                         soul_spark,
#                         "Post_Creator_Entanglement",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                 except Exception as vis_err:
...
#             current_stage_name = FLAG_HARMONICALLY_STRENGTHENED.replace(
#                 "_", " "
#             ).title()
#             logger.info(f"RootController Stage: {current_stage_name}...")
#             if VISUALIZATION_ENABLED:
#                 try:
#                     development_states.append(
#                         (soul_spark, "Pre_Harmonic_Strengthening")
#                     )
#                     visualize_soul_state(
#                         soul_spark,
#                         "Pre_Harmonic_Strengthening",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                 except Exception as vis_err:
...
#                 soul_spark=soul_spark,
#                 intensity=kwargs.get(
#                     "harmony_intensity", HARMONIC_STRENGTHENING_INTENSITY_DEFAULT
#                 ),
#                 duration_factor=kwargs.get(
#                     "harmony_duration_factor",
#                     HARMONIC_STRENGTHENING_DURATION_FACTOR_DEFAULT,
#                 ),
#             )
#             process_summary["stages_metrics"][current_stage_name] = hs_metrics
#             display_stage_metrics(current_stage_name, hs_metrics)
#             setattr(
#                 soul_spark, FLAG_READY_FOR_LIFE_CORD, True
#             )  # Set by perform_harmonic_strengthening
#             if VISUALIZATION_ENABLED:
#                 try:
#                     development_states.append(
#                         (soul_spark, "Post_Harmonic_Strengthening")
#                     )
#                     visualize_soul_state(
#                         soul_spark,
#                         "Post_Harmonic_Strengthening",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                 except Exception as vis_err:
...
#                 soul_spark=soul_spark,
#                 intensity=kwargs.get(
#                     "life_cord_intensity", 0.7
#                 ),  # Using the default passed to SoulCompletionController
...
#                 soul_spark=soul_spark,
#                 schumann_intensity=kwargs.get(
#                     "schumann_intensity", EARTH_HARMONY_INTENSITY_DEFAULT
#                 ),
#                 core_intensity=kwargs.get(
#                     "core_intensity", EARTH_HARMONY_INTENSITY_DEFAULT
#                 ),
#             )
...
#                 try:
#                     development_states.append(
#                         (soul_spark, "Pre_Identity_Crystallization")
#                     )
#                     visualize_soul_state(
#                         soul_spark,
...
#             _, id_metrics = perform_identity_crystallization(
#                 soul_spark=soul_spark, **id_kwargs
#             )
...
#                 try:
#                     development_states.append(
#                         (soul_spark, "Post_Identity_Crystallization")
#                     )
#                     visualize_soul_state(
#                         soul_spark,
...
#                         "nurturing_capacity": mother_resonance_data.get(
#                             "nurturing_capacity", 0.7
#                         ),
#                         "spiritual": mother_resonance_data.get(
#                             "spiritual", {"connection": 0.6}
#                         ),
#                         "love_resonance": mother_resonance_data.get(
#                             "love_resonance", 0.7
#                         ),
#                     }
#                 except Exception as mother_err:
#                     logger.warning(f"Failed to create mother profile: {mother_err}")
...
#                     comp_path = visualize_state_comparison(
#                         development_states,
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                     logger.info(
#                         f"Final development comparison visualization saved to: {comp_path}"
#                     )
#                 except Exception as vis_err:
#                     raise RuntimeError(
#                         f"Visualization error at {current_stage_name} (Post/Comparison): {vis_err}"
#                     ) from vis_err
...
#                 try:
#                     logger.info(
#                         f"Generating comprehensive report for soul {soul_spark.spark_id}..."
#                     )
#                     report_file_path = create_comprehensive_soul_report(
#                         soul_spark,
#                         "Soul_Development_Complete",
#                         VISUALIZATION_OUTPUT_DIR_ROOT,
#                         show=show_visuals,
#                     )
#                     logger.info(
#                         f"Comprehensive soul report saved to: {report_file_path}"
#                     )
#                     process_summary["comprehensive_report_path"] = report_file_path
#                 except Exception as report_err:
#                     logger.error(
#                         f"Failed to generate comprehensive soul report for {soul_spark.spark_id}: {report_err}",
#                         exc_info=True,
#                     )
...
#             process_summary["total_duration_seconds"] = (
#                 time.time() - single_soul_start_time
#             )
#             single_soul_final_summary = process_summary  # Since it's one soul
#             logger.info(
#                 f"===== Soul Processing Complete (ID: {soul_spark.spark_id}) (RootController) ====="
#             )
#             logger.info(
#                 f"Total time for soul: {process_summary['total_duration_seconds']:.2f}s. Incarnated={getattr(soul_spark, FLAG_INCARNATED, False)}"
#             )
...
#             logger.error(
#                 f"RootController: Soul processing (ID/BaseID: {soul_identifier_for_log}) failed at stage '{failed_stage_name_main}': {soul_err_main_loop}",
#                 exc_info=True,
#             )

#             process_summary_fail_main: Dict[str, Any] = {
#                 "base_id": base_id_str,  # Keep base_id for consistency if needed elsewhere
#                 "soul_id_on_fail": getattr(
#                     soul_spark, "spark_id", None
#                 ),  # Actual soul_id if available
#                 "success": False,
...
#             }
#             if soul_spark:
#                 process_summary_fail_main["final_soul_state_on_fail"] = (
#                     soul_spark.get_spark_metrics().get("core", {})
#                 )

#             if soul_spark and VISUALIZATION_ENABLED:
#                 stage_fail_name_vis_main = (
#                     f"Failed_At_{failed_stage_name_main.replace(' ', '_')}"
#                 )
#                 try:
#                     visualize_soul_state(
#                         soul_spark,
...
#                         show=show_visuals,
#                     )
#                 except Exception as vis_e_main:
#                     logger.error(
#                         f"RootController: Failed to visualize failed state: {vis_e_main}"
#                     )
...
#             logger.error(
#                 f"===== Soul (ID/BaseID: {soul_identifier_for_log}) Processing FAILED (RootController) ====="
#             )
#             print(
...
#         logger.info(
#             "RootController: Simulation loop finished. Generating final report for the soul..."
#         )
...
#                             else:
#                                 cleaned_list.append(round(item, 6))
...
#         final_report_path_for_soul = os.path.join(
#             report_path_base, soul_report_filename
#         )

#         if final_report_path_for_soul:
#             try:
#                 with open(final_report_path_for_soul, "w", encoding="utf-8") as f:
#                     json.dump(
#                         final_report_data_single_soul, f, cls=NumpyEncoder, indent=2
#                     )
#                 logger.info(
#                     f"RootController: Final report for soul {base_id_str} saved to {final_report_path_for_soul}"
#                 )
#             except Exception as report_err:
#                 logger.error(
#                     f"RootController: Failed save report for soul {base_id_str}: {report_err}",
#                     exc_info=True,
#                 )
#                 print(
#                     f"ERROR ROOT saving report for soul {base_id_str} to {final_report_path_for_soul}"
#                 )

#         print("\n" + "=" * 80 + "\nROOT SIMULATION COMPLETE (Single Soul)\n" + "=" * 80)
#         success_status = (
#             single_soul_final_summary.get("success", False)
#             if single_soul_final_summary
#             else False
#         )
#         print(f"Processed 1 soul | Duration: {time.time() - overall_start_time:.2f}s")
#         print(f"Success: {'Yes' if success_status else 'No'}")
#         if final_report_path_for_soul:
#             print(f"Report: {final_report_path_for_soul}")
...
#         logger.critical(
#             f"RootController: Simulation aborted due to critical error: {main_err}",
#             exc_info=True,
#         )
#         print(
#             "\n"
...
#             except Exception as persist_e:
#                 logger.error(
#                     f"RootController: ERROR persisting metrics at end of simulation: {persist_e}"
#                 )
...
#             "report_path_base": os.path.join(
#                 DATA_DIR_BASE, "reports", "simulation_runs"
#             ),
#             "show_visuals": False,
...
#             "harmony_duration_factor": HARMONIC_STRENGTHENING_DURATION_FACTOR_DEFAULT
#             * 0.9,
#             # Life Cord
...
# try:
#     from shared.constants.constants import *  # noqa: F403 - Allow wildcard import
