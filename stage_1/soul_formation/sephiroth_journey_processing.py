...
try:
    ...
    logger.warning(
        "Constants not loaded, using default INFO level for Sephiroth Journey logger."
    )
...
try:
    ...
    logger.critical(
        "CRITICAL ERROR: constants.py failed import in sephiroth_journey_processing.py"
    )
...
def calculate_resonance(freq1: float, freq2: float) -> float:
    ...
    logger.debug(
        f"  ResCalc (Enhanced): F1={freq1:.1f}, F2={freq2:.1f}, LogDiff={log_diff:.4f}, Main={main_resonance:.3f}, Harm={harmonic_resonance:.3f} -> Res={final_resonance:.4f}"
    )
    return max(0.0, min(1.0, final_resonance))


def calculate_geometric_resonance(
    soul_spark: SoulSpark, sephirah_influencer: SephirothField
) -> float:
    ...
    try:
        ...
        stability_factor = (
            getattr(soul_spark, "stability", 0.0) / const.MAX_STABILITY_SU
        )
        ...
        if soul_geom and sephirah_platonic:
            if soul_geom == sephirah_platonic:
                match_bonus = 0.3
        elif soul_geom is None and sephirah_platonic:
            match_bonus = 0.1
        elif soul_geom and sephirah_platonic is None:
            match_bonus = -0.1
        compatibility += match_bonus

        # Special resonance bonus
        if sephirah_name == "tiphareth" and layer_count >= 5:
            compatibility += 0.2
        ...
        raise RuntimeError(f"Geometric resonance calculation failed: {e}") from e


# --- Field Interaction Helper ---
def calculate_field_energy_transfer(
    source_potential_seu: float,
...
def _acquire_sephirah_aspects(
    ...
    sephirah_name = sephirah_influencer.sephirah_name
    logger.debug(
        f"Acquiring aspects from {sephirah_name} "
        f"(Resonance: {resonance_strength:.3f})..."
    )
    if not isinstance(sephirah_influencer, SephirothField):
        raise TypeError("sephirah_influencer invalid.")
    ...
    try:
        ...
        logger.debug(
            f"  Resonance strength: {resonance_strength:.3f}, Acquisition threshold: {acquisition_threshold:.3f}"
        )
        ...
        for aspect_name, details in sorted_aspects:
            ...
            variance = (
                random.uniform(-variance_range, variance_range)
                if variance_range > 0.01
                else 0
            )
            ...
            if seph_str < adjusted_threshold:
                logger.debug(
                    f"  Aspect '{aspect_name}' below threshold ({seph_str:.3f} < {adjusted_threshold:.3f}), skipping"
                )
                continue
            ...
            current_soul_strength = float(
                soul_current_aspects.get(aspect_name, {}).get("strength", 0.0)
            )

            # Add imparted strength, clamp to max
            new_soul_strength = min(
                const.MAX_ASPECT_STRENGTH, current_soul_strength + imparted_strength
            )
            ...
            if actual_gain > const.FLOAT_EPSILON:  # If change occurred
                ...
                if (
                    aspect_name not in soul_current_aspects
                    or current_soul_strength <= const.FLOAT_EPSILON
                ):
                    # Gain new aspect
                    ...
                    gained_count += 1
                    logger.info(
                        f"Gained NEW aspect '{aspect_name}' from "
                    ...
                    soul_current_aspects[aspect_name]["last_strengthened"] = (
                        transfer_time
                    )
                    soul_current_aspects[aspect_name]["details"].update(
                        details_copy
                    )  # Merge details
                    soul_current_aspects[aspect_name]["resonance_factor"] = (
                        resonance_factor  # Update
                    )
                    strengthened_count += 1
                    logger.info(
                        f"STRENGTHENED aspect '{aspect_name}' "
        ...
        if (
            resonance_strength > const.FLOAT_EPSILON
            and gained_count == 0
            and strengthened_count == 0
        ):
            ...
            if primary_aspects:
                ...
                for aspect_to_gain in primary_aspects:
                    if aspect_to_gain in detailed_aspects:
                        ...
                        curr_str = float(
                            soul_current_aspects.get(aspect_to_gain, {}).get(
                                "strength", 0.0
                            )
                        )
                        new_str = min(
                            const.MAX_ASPECT_STRENGTH, curr_str + min_imparted
                        )
                        ...
                        if new_str > curr_str + const.FLOAT_EPSILON:
                            ...
                            gained_count = 1
                            logger.info(
                                f"Guaranteed minimum acquisition of '{aspect_to_gain}' "
        ...
        logger.info(
            f"Aspect acquisition from {sephirah_name}: "
        ...
        logger.error(
            f"Error acquiring aspects from {sephirah_name}: {e}", exc_info=True
        )
        raise RuntimeError(f"Aspect acquisition failed: {e}") from e


# --- Geometric Transformation Helper ---
def _apply_geometric_transformation(
    soul_spark: SoulSpark,
    sephirah_influencer: SephirothField,
    resonance_strength: float,
) -> Dict[str, float]:
    ...
    try:
        ...
        if not plat_affinity:
            return {}  # No geometry defined

        effects_to_apply = const.GEOMETRY_EFFECTS.get(
            plat_affinity, const.DEFAULT_GEOMETRY_EFFECT
        )
        ...
        for effect_name, modifier in effects_to_apply.items():
            ...
            if is_boost:
                target_attr = effect_name.split("_boost")[0].split("_factor")[0]
            if is_push:
                target_attr = effect_name.split("_push")[0]

            if not hasattr(soul_spark, target_attr):
                logger.warning(f"Geom target '{target_attr}' not on SoulSpark.")
                continue
            ...
            if target_attr == "stability":
                max_clamp = const.MAX_STABILITY_SU
            elif target_attr == "coherence":
                max_clamp = const.MAX_COHERENCE_CU
        ...
        if transformation_occurred:
            ...
            logger.debug(
                f"Applied geom effects to soul {soul_spark.spark_id} from {sephirah_name}."
            )
        return changes_applied_dict
    except Exception as e:
        logger.error(
            f"Error applying geometric transform for {sephirah_name}: {e}",
            exc_info=True,
        )
...
def record_interaction_failure(
    soul_id: str, sephirah: str, timestamp: str, error_type: str, error_details: str
) -> None:
...
def _develop_layer_resonance(
    soul_spark: SoulSpark, target_frequency: float, resonance_strength: float
) -> Dict[str, Any]:
    """
    ...
    if not hasattr(soul_spark, "layers") or not soul_spark.layers:
        logger.error(
            f"Cannot develop layer resonance: Soul {soul_spark.spark_id} has no layers"
        )
    ...
    if base_freq <= const.FLOAT_EPSILON:
        logger.error(
            f"Invalid base frequency {base_freq} for soul {soul_spark.spark_id}"
        )
    ...
    for n in range(1, 6):
        for d in range(1, 6):
            ...
            deviation = abs(harmonic_freq - target_frequency) / max(
                target_frequency, const.FLOAT_EPSILON
            )
    ...
    # Store in layer data
    latest_layer["harmonic_data"][str(len(latest_layer["harmonic_data"]))] = (
        harmonic_data
    )
    ...
    if "uniformity" in latest_layer["density"] and resonance_strength > 0.7:
        ...
        uniformity_boost = max(
            0.0, 0.1 * resonance_strength * (1.0 - current_uniformity)
        )
    ...
    logger.debug(
        f"Layer resonance developed: Ratio={best_match['ratio']}, "
...
def _form_sephirah_layer(
    ...
    try:
        ...
        energy_transfer = calculate_field_energy_transfer(
            sephirah_potential_seu,
            soul_spark.energy,
            coupling,
            const.SEPHIROTH_ENERGY_EXCHANGE_RATE_K,
            delta_time=1.0,  # Use rate constant
        )
        soul_spark.energy = min(
            const.MAX_SOUL_ENERGY_SEU, max(0.0, soul_spark.energy + energy_transfer)
        )
        ...
        changes["sephirah_influence_increment"] = (
            influence_increment  # Record increment
        )

        logger.debug(
            f"  Sephiroth influence: {current_influence:.3f} -> {new_influence:.3f} (+{influence_increment:.3f})"
        )

        # 5. Optimize Phase Coherence if resonance is strong
        phase_improvement = 0.0
        if resonance_strength > 0.2 and hasattr(
            soul_spark, "_optimize_phase_coherence"
        ):
        ...
        geom_changes = _apply_geometric_transformation(
            soul_spark, sephirah_influencer, resonance_strength
        )
        changes["geometric_effects"].update(
            geom_changes
        )  # Update geometric_effects dict

        # 7. Store Interaction Metadata
        interaction_data = {
            "type": "sephirah_layer_formation",
            "timestamp": timestamp,
            ...
            "layer_vibration": vibration_factor,
        ...
        logger.debug(
            f"Formed layer for {sephirah_name}. "
        ...
        logger.error(
            f"Error forming Sephirah layer for {sephirah_name}: {e}", exc_info=True
        )
        raise RuntimeError(f"Layer formation failed for {sephirah_name}: {e}")


def process_sephirah_interaction(
    soul_spark: SoulSpark,
    sephirah_influencer: SephirothField,
    ...
    if not isinstance(sephirah_influencer, SephirothField):
        raise TypeError("sephirah_influencer invalid.")
    if not isinstance(duration, (int, float)) or duration <= const.FLOAT_EPSILON:
    ...
    log_start = (
        f"--- Starting SephInteraction [Natural Principles]: "
    ...
    try:
        ...
        freq_res = calculate_resonance(
            soul_spark.frequency, sephirah_influencer.target_frequency
        )
        ...
        resonance_strength = (
            ...
            + (freq_res * geom_res * const.GOLDEN_RATIO)
            * const.SEPHIROTH_JOURNEY_RESONANCE_WEIGHT_PHI
        ) / total_weight
        resonance_strength = max(0.0, min(1.0, resonance_strength))  # Clamp 0-1
        logger.info(
            f"  {sephirah_name}: Resonance - Freq={freq_res:.3f}, Geom={geom_res:.3f} -> Total={resonance_strength:.4f}"
        )
        ...
        logger.info(
            f"  {sephirah_name}: Step 3: Forming layer, applying energy/influence/geometry..."
        )
        layer_changes = _form_sephirah_layer(
            soul_spark, sephirah_influencer, resonance_strength
        )
        log_layer = (
            f"  {sephirah_name}: Layer     - "
        ...
        if layer_changes.get("geometric_effects"):
            logger.info(
                f"  {sephirah_name}: Geometry  - Changes: {layer_changes['geometric_effects']}"
            )
        ...
        if layer_resonance_metrics:
            ...
            logger.info(
                f"  {sephirah_name}: Resonance - Ratio={layer_ratio}, "
        ...
        step_changes = {
            ...
            "sephirah_influence_increment": layer_changes.get(
                "sephirah_influence_increment", 0.0
            ),
            "phase_coherence_improvement": layer_changes.get(
                "phase_coherence_improvement", 0.0
            ),
        ...
        logger.error(
            f"Error during Sephirah interaction for {soul_spark.spark_id} in "
            f"{sephirah_name}: {e}",
            exc_info=True,
        )
        raise RuntimeError(f"Sephirah interaction failed for {sephirah_name}: {e}") from e
