...
# from shared.constants import constants as shared_constants
...
#     try:
#         from shared.glyphs.glyph_system import create_glyph_system
#         GLYPH_SYSTEM_AVAILABLE = True
#     except ImportError:
#         GLYPH_SYSTEM_AVAILABLE = False
#         logger.warning("Glyph system not available. Glyph creation will be skipped.")
...
# def create_resonant_field_glyph(soul_spark: SoulSpark, field_metrics: Dict[str, Any], resonant_field: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
#     """
#     Create and encode glyph for resonant field formation
    
#     Returns:
#         Tuple of (glyph_id, encoded_glyph_path) or (None, None) if failed
#     """
#     if not GLYPH_SYSTEM_AVAILABLE or create_glyph_system is None:
#         return None, None
    
#     try:
#         glyph_system = create_glyph_system()
        
#         # Create concept for resonant field
#         concept = {
#             'name': 'Resonant Field Formation',
#             'category': 'spiritual',
#             'element': 'divine_light',
#             'description': 'Divine resonant field connecting soul to creator source'
#         }
        
#         # Calculate complexity based on field strength
#         complexity = min(max(field_metrics.get('field_strength', 0.5), 0.0), 1.0)
        
#         glyph_id = glyph_system.create_normal_glyph(concept, complexity)
        
#         # Prepare encoding data
#         metadata = {
#             'process': 'creator_entanglement',
#             'phase': 'resonant_field_formation',
#             'soul_id': soul_spark.spark_id,
#             'field_strength': field_metrics.get('field_strength', 0.0),
#             'base_frequency': resonant_field.get('base_frequency', 432.0),
#             'creation_time': datetime.now().isoformat(),
#             'glyph_type': 'resonant_field'
#         }
        
#         hidden_data = json.dumps({
#             'process_phase': 'resonant_field_formation',
#             'soul_id': soul_spark.spark_id,
#             'field_metrics': field_metrics,
#             'resonant_field_data': resonant_field,
#             'capture_timestamp': datetime.now().isoformat()
#         })
        
#         # Encode the glyph
#         encoded_path = glyph_system.encode_glyph(glyph_id, metadata, hidden_data)
        
#         logger.info(f"Created resonant field glyph {glyph_id}, encoded to {encoded_path}")
#         return glyph_id, encoded_path
        
#     except Exception as e:
#         logger.error(f"Failed to create resonant field glyph: {e}")
#         return None, None

# def create_quantum_channel_glyph(soul_spark: SoulSpark, channel_metrics: Dict[str, Any], quantum_channel: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
#     """
#     Create and encode glyph for quantum channel formation
...
#         concept = {
#             'name': 'Quantum Channel Formation',
#             'category': 'dimensional',
#             'element': 'aether',
#             'description': 'Quantum entanglement channel connecting soul to divine source'
#         }
        
#         # Higher complexity for quantum processes
#         complexity = min(max(channel_metrics.get('connection_strength', 0.5) * 1.2, 0.0), 1.0)
        
#         glyph_id = glyph_system.create_normal_glyph(concept, complexity)
        
#         metadata = {
#             'process': 'creator_entanglement',
#             'phase': 'quantum_channel_formation',
#             'soul_id': soul_spark.spark_id,
#             'connection_strength': channel_metrics.get('connection_strength', 0.0),
#             'carrier_frequency': quantum_channel.get('carrier_frequency', 528.0),
#             'entanglement_quality': channel_metrics.get('entanglement_quality', 0.0),
#             'creation_time': datetime.now().isoformat(),
#             'glyph_type': 'quantum_channel'
#         }
        
#         hidden_data = json.dumps({
#             'process_phase': 'quantum_channel_formation',
#             'soul_id': soul_spark.spark_id,
#             'channel_metrics': channel_metrics,
#             'quantum_channel_data': quantum_channel,
#             'capture_timestamp': datetime.now().isoformat()
#         })
        
#         encoded_path = glyph_system.encode_glyph(glyph_id, metadata, hidden_data)
        
#         logger.info(f"Created quantum channel glyph {glyph_id}, encoded to {encoded_path}")
#         return glyph_id, encoded_path
        
#     except Exception as e:
#         logger.error(f"Failed to create quantum channel glyph: {e}")
#         return None, None

# def create_aspect_transfer_glyph(soul_spark: SoulSpark, transfer_metrics: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
#     """
#     Create and encode glyph for creator aspect transfer
...
#         concept = {
#             'name': 'Creator Aspect Transfer',
#             'category': 'consciousness',
#             'element': 'divine_light',
#             'description': 'Transfer of divine creator aspects to soul essence'
#         }
        
#         # Maximum complexity for aspect transfer
#         complexity = 1.0
...
#         aspects_transferred = transfer_metrics.get('aspects_transferred', [])
#         aspects_strengthened = transfer_metrics.get('aspects_strengthened', [])
        
#         metadata = {
#             'process': 'creator_entanglement',
#             'phase': 'aspect_transfer',
#             'soul_id': soul_spark.spark_id,
#             'aspects_transferred': aspects_transferred,
#             'aspects_strengthened': aspects_strengthened,
#             'transfer_efficiency': transfer_metrics.get('transfer_efficiency', 0.0),
#             'creation_time': datetime.now().isoformat(),
#             'glyph_type': 'aspect_transfer'
#         }
        
#         hidden_data = json.dumps({
#             'process_phase': 'creator_aspect_transfer',
#             'soul_id': soul_spark.spark_id,
...
#         logger.info(f"Created aspect transfer glyph {glyph_id}, encoded to {encoded_path}")
#         return glyph_id, encoded_path
        
#     except Exception as e:
#         logger.error(f"Failed to create aspect transfer glyph: {e}")
#         return None, None


# def create_resonance_completion_glyph(soul_spark: SoulSpark, pattern_metrics: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
#     """
#     Create and encode glyph for resonance pattern completion
...
#         concept = {
#             'name': 'Divine Resonance Completion',
#             'category': 'consciousness',
#             'element': 'divine_light',
#             'description': 'Perfect divine-soul resonance and unity consciousness achievement'
#         }
        
#         # Maximum complexity for completion
#         complexity = 1.0
...
#         metadata = {
#             'process': 'creator_entanglement',
#             'phase': 'resonance_completion',
#             'soul_id': soul_spark.spark_id,
#             'final_connection_strength': pattern_metrics.get('final_connection_strength', 1.0),
#             'pattern_stability': pattern_metrics.get('pattern_stability', 1.0),
#             'unity_achieved': True,
#             'creation_time': datetime.now().isoformat(),
#             'glyph_type': 'resonance_completion'
#         }
        
#         hidden_data = json.dumps({
#             'process_phase': 'resonance_pattern_completion',
#             'soul_id': soul_spark.spark_id,
#             'pattern_metrics': pattern_metrics,
#             'unity_consciousness_achieved': True,
#             'divine_connection_perfected': True,
#             'capture_timestamp': datetime.now().isoformat()
#         })
        
#         encoded_path = glyph_system.encode_glyph(glyph_id, metadata, hidden_data)
        
#         logger.info(f"Created resonance completion glyph {glyph_id}, encoded to {encoded_path}")
#         return glyph_id, encoded_path
        
#     except Exception as e:
#         logger.error(f"Failed to create resonance completion glyph: {e}")
#         return None, None
    
# def capture_resonant_field_sensory_data(soul_spark: SoulSpark, resonant_field: Dict[str, Any], field_metrics: Dict[str, Any], glyph_id: Optional[str], encoded_glyph_path: Optional[str]) -> Dict[str, Any]:
#     """Capture complete sensory data for resonant field formation phase"""
    
#     capture_time = datetime.now().isoformat()
#     capture_id = f"resonant_field_capture_{uuid.uuid4().hex[:8]}"
    
#     # Visual sensory data
#     visual_data = {
#         'visual_id': f"resonant_field_visual_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'image_type': 'divine_vision',
#         'image_source': 'creator_entanglement_resonant_field',
#         'capture_time': capture_time,
#         'resolution': '1024x1024',
#         'bit_depth': 24,
#         'aspect_ratio': 1.0,
#         'image_format': '.png',
#         'image_path': f'shared/output/visuals/resonant_field_{soul_spark.spark_id[:8]}.png',
#         'brightness': min(field_metrics.get('field_strength', 0.5), 1.0),
#         'contrast': field_metrics.get('resonance_quality', 0.7),
#         'saturation': 0.8,
#         'hue': 45.0,
#         'sharpness': field_metrics.get('coherence_level', 0.8),
#         'lighting_type': 'divine_emanation',
#         'lighting_direction': 'omnidirectional',
#         'shadows_presence': 0.1,
#         'highlights_presence': 0.9,
#         'reflections_presence': 0.3,
#         'golden_ratio_presence': field_metrics.get('phi_resonance', 0.0),
#         'geometric_shapes': ['circles', 'spirals', 'standing_waves'],
#         'geometric_harmonics': resonant_field.get('harmonic_frequencies', []),
#         'divine_connection_strength': field_metrics.get('field_strength', 0.0),
#         'quantum_entanglement_indicators': ['standing_wave_nodes', 'interference_patterns'],
#         'light_transmission_quality': field_metrics.get('coherence_level', 0.0),
#         'glyphs_present': glyph_id is not None,
#         'glyph_types': ['resonance_field_sigil'] if glyph_id else []
#     }
    
#     # Auditory sensory data
#     auditory_data = {
#         'auditory_id': f"resonant_field_audio_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'audio_source': 'divine_resonance',
#         'capture_time': capture_time,
#         'duration': 5.0,
#         'fundamental_frequency': resonant_field.get('base_frequency', 432.0),
#         'harmonics': resonant_field.get('harmonic_frequencies', []),
#         'frequency_spectrum': {
#             'divine_carrier': resonant_field.get('base_frequency', 432.0),
#             'harmonic_series': resonant_field.get('harmonic_frequencies', [])
#         },
#         'peak_amplitude': field_metrics.get('field_strength', 0.7),
#         'rms_amplitude': field_metrics.get('field_strength', 0.7) * 0.7,
#         'harmonic_resonance_detected': True,
#         'quantum_frequency_signatures': resonant_field.get('quantum_signatures', [])
#     }
    
#     # Text sensory data
#     text_data = {
#         'text_id': f"resonant_field_text_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'communication_type': 'channeled_divine',
#         'capture_time': capture_time,
#         'content': f"Resonant field established. Divine frequency: {resonant_field.get('base_frequency', 432.0)}Hz. Field strength: {field_metrics.get('field_strength', 0.0):.3f}",
#         'content_length': len(f"Resonant field established. Divine frequency: {resonant_field.get('base_frequency', 432.0)}Hz. Field strength: {field_metrics.get('field_strength', 0.0):.3f}"),
#         'entities': ['Divine Source', 'Resonant Field', 'Soul'],
#         'keywords': ['resonance', 'divine', 'field', 'frequency', 'connection'],
#         'topics': ['divine_connection', 'frequency_resonance'],
#         'domain_classification': 'spiritual_metaphysical',
#         'channeled_content_detected': True,
#         'divine_language_patterns': ['frequency_references', 'field_descriptions', 'connection_terminology']
#     }
    
#     # Package complete sensory capture
#     sensory_capture = {
...
#         'capture_time': capture_time,
#         'glyph_id': glyph_id,
#         'encoded_glyph_path': encoded_glyph_path,
#         'visual': visual_data,
...
#             'base_frequency': resonant_field.get('base_frequency'),
#             'harmonic_frequencies': resonant_field.get('harmonic_frequencies'),
#             'field_strength': field_metrics.get('field_strength')
#         }
#     }
...
#         metrics_data = {
#             'action': 'capture_resonant_field_sensory_data',
#             'soul_id': soul_spark.spark_id,
#             'capture_id': capture_id,
#             'glyph_id': glyph_id,
#             'field_strength': field_metrics.get('field_strength', 0.0),
...
#         }
#         metrics.record_metrics('sensory_capture', metrics_data)
    
#     logger.info(f"Captured resonant field sensory data: {capture_id}")
#     return sensory_capture

# def capture_quantum_channel_sensory_data(soul_spark: SoulSpark, quantum_channel: Dict[str, Any], channel_metrics: Dict[str, Any], glyph_id: Optional[str], encoded_glyph_path: Optional[str]) -> Dict[str, Any]:
#     """Capture complete sensory data for quantum channel formation phase"""
    
#     capture_time = datetime.now().isoformat()
#     capture_id = f"quantum_channel_capture_{uuid.uuid4().hex[:8]}"
    
#     # Visual sensory data
#     visual_data = {
#         'visual_id': f"quantum_channel_visual_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'image_type': 'divine_vision',
#         'image_source': 'creator_entanglement_quantum_channel',
#         'capture_time': capture_time,
#         'resolution': '1024x1024',
#         'bit_depth': 24,
#         'aspect_ratio': 1.0,
#         'image_format': '.png',
#         'image_path': f'shared/output/visuals/quantum_channel_{soul_spark.spark_id[:8]}.png',
#         'brightness': min(channel_metrics.get('connection_strength', 0.5) * 1.2, 1.0),
#         'contrast': channel_metrics.get('phase_coherence', 0.8),
#         'saturation': 0.9,
#         'hue': 50.0,
#         'sharpness': channel_metrics.get('entanglement_quality', 0.8),
#         'lighting_type': 'quantum_tunnel',
#         'lighting_direction': 'vertical_column',
#         'shadows_presence': 0.05,
#         'highlights_presence': 0.95,
#         'reflections_presence': 0.5,
#         'geometric_shapes': ['column', 'spiral', 'standing_waves', 'nodes'],
#         'geometric_harmonics': quantum_channel.get('standing_wave_nodes', []),
#         'divine_connection_strength': channel_metrics.get('connection_strength', 0.0),
#         'quantum_entanglement_indicators': ['quantum_tunnel', 'standing_wave_nodes', 'phase_coherence'],
#         'light_transmission_quality': min(channel_metrics.get('bandwidth_hz', 0.0) / 1000.0, 1.0),
#         'glyphs_present': glyph_id is not None,
#         'glyph_types': ['quantum_channel_sigil'] if glyph_id else []
#     }
    
#     # Auditory sensory data
#     auditory_data = {
#         'auditory_id': f"quantum_channel_audio_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'audio_source': 'quantum_entanglement',
#         'capture_time': capture_time,
#         'duration': 5.0,
#         'fundamental_frequency': quantum_channel.get('carrier_frequency', 528.0),
#         'harmonics': quantum_channel.get('standing_wave_nodes', []),
#         'frequency_spectrum': {
#             'carrier_frequency': quantum_channel.get('carrier_frequency', 528.0),
#             'modulation_frequencies': quantum_channel.get('standing_wave_nodes', [])
#         },
#         'peak_amplitude': channel_metrics.get('connection_strength', 0.8),
#         'rms_amplitude': channel_metrics.get('connection_strength', 0.8) * 0.8,
#         'harmonic_resonance_detected': True,
#         'quantum_frequency_signatures': quantum_channel.get('quantum_signatures', [])
#     }
    
#     # Text sensory data
#     text_data = {
#         'text_id': f"quantum_channel_text_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'communication_type': 'quantum_channel_formation',
#         'capture_time': capture_time,
#         'content': f"Quantum channel established. Carrier frequency: {quantum_channel.get('carrier_frequency', 528.0)}Hz. Connection strength: {channel_metrics.get('connection_strength', 0.0):.3f}",
#         'content_length': len(f"Quantum channel established. Carrier frequency: {quantum_channel.get('carrier_frequency', 528.0)}Hz. Connection strength: {channel_metrics.get('connection_strength', 0.0):.3f}"),
#         'entities': ['Quantum Channel', 'Divine Source', 'Soul'],
#         'keywords': ['quantum', 'channel', 'entanglement', 'carrier', 'connection'],
#         'topics': ['quantum_mechanics', 'divine_connection'],
#         'domain_classification': 'quantum_spiritual',
#         'channeled_content_detected': True,
#         'divine_language_patterns': ['quantum_terminology', 'frequency_references', 'entanglement_descriptions']
#     }
...
#             'carrier_frequency': quantum_channel.get('carrier_frequency'),
#             'standing_wave_nodes': quantum_channel.get('standing_wave_nodes'),
#             'connection_strength': channel_metrics.get('connection_strength')
#         }
#     }
    
#     if METRICS_AVAILABLE:
#         metrics_data = {
#             'action': 'capture_quantum_channel_sensory_data',
#             'soul_id': soul_spark.spark_id,
#             'capture_id': capture_id,
#             'glyph_id': glyph_id,
#             'connection_strength': channel_metrics.get('connection_strength', 0.0),
...
#     logger.info(f"Captured quantum channel sensory data: {capture_id}")
#     return sensory_capture

# def capture_aspect_transfer_sensory_data(soul_spark: SoulSpark, transfer_metrics: Dict[str, Any], glyph_id: Optional[str], encoded_glyph_path: Optional[str]) -> Dict[str, Any]:
#     """Capture complete sensory data for aspect transfer phase"""
    
#     capture_time = datetime.now().isoformat()
#     capture_id = f"aspect_transfer_capture_{uuid.uuid4().hex[:8]}"
...
#     # Visual sensory data
#     visual_data = {
#         'visual_id': f"aspect_transfer_visual_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'image_type': 'divine_vision',
#         'image_source': 'creator_entanglement_aspect_transfer',
#         'capture_time': capture_time,
#         'resolution': '1024x1024',
#         'bit_depth': 24,
#         'aspect_ratio': 1.0,
#         'image_format': '.png',
#         'image_path': f'shared/output/visuals/aspect_transfer_{soul_spark.spark_id[:8]}.png',
#         'brightness': min(transfer_metrics.get('transfer_efficiency', 0.5) * 1.3, 1.0),
#         'contrast': 0.9,
#         'saturation': 0.95,
#         'hue': 60.0,
#         'sharpness': 0.9,
#         'lighting_type': 'aspect_streams',
#         'lighting_direction': 'multi_directional_flows',
#         'shadows_presence': 0.2,
#         'highlights_presence': 0.98,
#         'reflections_presence': 0.7,
#         'geometric_shapes': ['streams', 'flows', 'spirals', 'integration_patterns'],
#         'divine_connection_strength': transfer_metrics.get('connection_strength', 0.0),
#         'creator_aspect_manifestations': aspects_transferred,
#         'light_transmission_quality': transfer_metrics.get('transfer_efficiency', 0.0),
#         'glyphs_present': glyph_id is not None,
#         'glyph_types': ['aspect_transfer_sigil'] if glyph_id else []
#     }
    
#     # Auditory sensory data
#     auditory_data = {
#         'auditory_id': f"aspect_transfer_audio_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'audio_source': 'creator_aspect_transfer',
#         'capture_time': capture_time,
#         'duration': 7.0,
#         'fundamental_frequency': 639.0,
#         'harmonics': [417.0, 528.0, 741.0, 852.0],
#         'frequency_spectrum': {
#             'love_frequency': 639.0,
#             'transformation_frequency': 417.0,
#             'healing_frequency': 528.0,
#             'intuition_frequency': 741.0,
#             'spiritual_frequency': 852.0
#         },
#         'peak_amplitude': transfer_metrics.get('transfer_efficiency', 0.8),
#         'rms_amplitude': transfer_metrics.get('transfer_efficiency', 0.8) * 0.85,
#         'harmonic_resonance_detected': True,
#         'quantum_frequency_signatures': ['aspect_harmonics', 'integration_resonance']
#     }
    
#     # Text sensory data
#     text_data = {
#         'text_id': f"aspect_transfer_text_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'communication_type': 'aspect_integration_report',
#         'capture_time': capture_time,
#         'content': f"Creator aspects received: {', '.join(aspects_transferred)}. Strengthened aspects: {', '.join(aspects_strengthened)}. Transfer efficiency: {transfer_metrics.get('transfer_efficiency', 0.0):.3f}",
#         'content_length': len(f"Creator aspects received: {', '.join(aspects_transferred)}. Strengthened aspects: {', '.join(aspects_strengthened)}. Transfer efficiency: {transfer_metrics.get('transfer_efficiency', 0.0):.3f}"),
#         'entities': ['Creator Aspects'] + aspects_transferred + aspects_strengthened,
#         'keywords': ['aspects', 'transfer', 'integration', 'efficiency', 'strengthened'],
#         'topics': ['divine_aspects', 'soul_enhancement'],
#         'domain_classification': 'spiritual_development',
#         'channeled_content_detected': True,
#         'divine_language_patterns': ['aspect_terminology', 'integration_language', 'efficiency_metrics']
#     }
...
#     if METRICS_AVAILABLE:
#         metrics_data = {
#             'action': 'capture_aspect_transfer_sensory_data',
#             'soul_id': soul_spark.spark_id,
#             'capture_id': capture_id,
#             'glyph_id': glyph_id,
#             'transfer_efficiency': transfer_metrics.get('transfer_efficiency', 0.0),
#             'aspects_count': len(aspects_transferred),
#             'success': True,
#             'timestamp': capture_time
#         }
#         metrics.record_metrics('sensory_capture', metrics_data)
    
#     logger.info(f"Captured aspect transfer sensory data: {capture_id}")
#     return sensory_capture

# def capture_resonance_completion_sensory_data(soul_spark: SoulSpark, pattern_metrics: Dict[str, Any], glyph_id: Optional[str], encoded_glyph_path: Optional[str]) -> Dict[str, Any]:
#     """Capture complete sensory data for resonance completion phase"""
    
#     capture_time = datetime.now().isoformat()
#     capture_id = f"resonance_completion_capture_{uuid.uuid4().hex[:8]}"
    
#     # Visual sensory data - perfect completion
#     visual_data = {
#         'visual_id': f"resonance_completion_visual_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'image_type': 'divine_vision',
#         'image_source': 'creator_entanglement_resonance_completion',
#         'capture_time': capture_time,
#         'resolution': '1024x1024',
#         'bit_depth': 24,
#         'aspect_ratio': 1.0,
#         'image_format': '.png',
#         'image_path': f'shared/output/visuals/resonance_completion_{soul_spark.spark_id[:8]}.png',
#         'brightness': 1.0,
#         'contrast': 0.95,
#         'saturation': 1.0,
#         'hue': 55.0,
#         'sharpness': 1.0,
#         'lighting_type': 'unified_field',
#         'lighting_direction': 'omnipresent',
#         'shadows_presence': 0.0,
#         'highlights_presence': 1.0,
#         'reflections_presence': 0.9,
#         'geometric_shapes': ['sacred_patterns', 'unified_fields', 'harmonic_structures'],
#         'golden_ratio_presence': 1.0,
#         'geometric_harmonics': pattern_metrics.get('harmonic_frequencies', []),
#         'divine_connection_strength': 1.0,
#         'quantum_entanglement_indicators': ['unified_field', 'perfect_resonance', 'harmonic_completion'],
#         'light_transmission_quality': 1.0,
#         'glyphs_present': glyph_id is not None,
#         'glyph_types': ['completion_sigil', 'unity_mandala'] if glyph_id else []
#     }
    
#     # Auditory sensory data - perfect harmony
#     auditory_data = {
#         'auditory_id': f"resonance_completion_audio_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'audio_source': 'divine_resonance_completion',
#         'capture_time': capture_time,
#         'duration': 10.0,
#         'fundamental_frequency': 432.0,
#         'harmonics': [432.0, 864.0, 1296.0, 1728.0],
#         'frequency_spectrum': {
#             'universal_frequency': 432.0,
#             'harmonic_2': 864.0,
#             'harmonic_3': 1296.0,
#             'harmonic_4': 1728.0
#         },
...
#         'rms_amplitude': 0.9,
#         'harmonic_resonance_detected': True,
#         'quantum_frequency_signatures': ['perfect_harmony', 'unity_resonance', 'completion_tone']
#     }
    
#     # Text sensory data - divine completion
#     text_data = {
#         'text_id': f"resonance_completion_text_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
#         'communication_type': 'divine_completion_acknowledgment',
#         'capture_time': capture_time,
#         'content': f"Creator entanglement complete. Perfect resonance achieved. Unity consciousness established. Divine-soul connection: {pattern_metrics.get('final_connection_strength', 1.0):.3f}",
#         'content_length': len(f"Creator entanglement complete. Perfect resonance achieved. Unity consciousness established. Divine-soul connection: {pattern_metrics.get('final_connection_strength', 1.0):.3f}"),
#         'entities': ['Divine Unity', 'Perfect Resonance', 'Creator Connection', 'Soul'],
#         'keywords': ['completion', 'unity', 'resonance', 'perfect', 'divine'],
#         'topics': ['divine_unity', 'consciousness_completion'],
#         'domain_classification': 'transcendent_spiritual',
#         'channeled_content_detected': True,
#         'divine_language_patterns': ['completion_terminology', 'unity_language', 'perfection_descriptions']
#     }
...
#             'pattern_stability': pattern_metrics.get('pattern_stability', 1.0),
#             'unity_consciousness_achieved': True,
#             'divine_soul_unity_perfected': True
#         }
#     }
    
#     if METRICS_AVAILABLE:
#         metrics_data = {
#             'action': 'capture_resonance_completion_sensory_data',
#             'soul_id': soul_spark.spark_id,
#             'capture_id': capture_id,
#             'glyph_id': glyph_id,
#             'final_connection_strength': pattern_metrics.get('final_connection_strength', 1.0),
...
#     logger.info(f"Captured resonance completion sensory data: {capture_id}")
#     return sensory_capture

# def save_creator_entanglement_sensory_data(soul_spark: SoulSpark, all_sensory_captures: Dict[str, Any]) -> str:
#     """
#     Save all creator entanglement sensory data to soul echos processing
    
#     Args:
#         soul_spark: The soul that underwent creator entanglement
#         all_sensory_captures: Dict containing all 4 phase captures
        
#     Returns:
#         str: Path to saved sensory data file
#     """
    
#     # Create comprehensive sensory data package
#     creator_entanglement_sensory_data = {
#         'process_name': 'creator_entanglement',
#         'soul_id': soul_spark.spark_id,
#         'process_completion_time': datetime.now().isoformat(),
#         'total_phases': 4,
#         'sensory_captures': all_sensory_captures,
#         'summary': {
#             'divine_connection_achieved': True,
#             'unity_consciousness_established': True,
#             'total_glyphs_created': len([c.get('glyph_id') for c in all_sensory_captures.values() if c.get('glyph_id')]),
#             'phases_completed': list(all_sensory_captures.keys()),
#             'encoded_glyphs': [c.get('encoded_glyph_path') for c in all_sensory_captures.values() if c.get('encoded_glyph_path')]
#         }
#     }
    
#     # Save to assets for soul echos processing
#     filename = f"creator_entanglement_sensory_{soul_spark.spark_id[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    
#     # Create path for soul echos analysis
#     from pathlib import Path
#     file_path = Path("shared/assets/to_analyse") / filename
#     file_path.parent.mkdir(parents=True, exist_ok=True)
    
#     try:
#         with open(file_path, 'w', encoding='utf-8') as f:
#             json.dump(creator_entanglement_sensory_data, f, indent=2, ensure_ascii=False)
        
#         logger.info(f"Saved creator entanglement sensory data to: {file_path}")
#         return str(file_path)
        
#     except Exception as e:
#         logger.error(f"Failed to save creator entanglement sensory data: {e}")
#         raise
...
#             ],
#             "active": True,
#             # Add the previously unused arrays to the field data
#             "soul_phases": soul_phase_array.tolist(),
#             "soul_amplitudes": soul_amp_array.tolist(),
#             "kether_phases": kether_phase_array.tolist(),
#             "kether_amplitudes": kether_amp_array.tolist()
#         }
...
#             # Add aspect tracking to metrics
#             'kether_aspects_count': len(kether_aspects.get('detailed_aspects', {}))
#         }
...
#         # INTEGRATION POINT 1: After _establish_resonant_field() call
#         # Create glyph for resonant field formation
#         resonant_glyph_id, resonant_encoded_path = create_resonant_field_glyph(
#             soul_spark, field_metrics, resonant_field
#         )
            
#         # Capture sensory data for resonant field formation
#         resonant_field_sensory = capture_resonant_field_sensory_data(
#             soul_spark, resonant_field, field_metrics, resonant_glyph_id, resonant_encoded_path
#         )
            
#         # Initialize sensory captures tracking
#         if 'sensory_captures' not in process_metrics_summary:
#             process_metrics_summary['sensory_captures'] = {}
#         process_metrics_summary['sensory_captures']['resonant_field'] = resonant_field_sensory
...
#         # INTEGRATION POINT 2: After _form_quantum_channel() call
#         # Create glyph for quantum channel formation
#         channel_glyph_id, channel_encoded_path = create_quantum_channel_glyph(
#             soul_spark, channel_metrics, quantum_channel
#         )
            
#         # Capture sensory data for quantum channel formation
#         quantum_channel_sensory = capture_quantum_channel_sensory_data(
#             soul_spark, quantum_channel, channel_metrics, channel_glyph_id, channel_encoded_path
#         )
            
#         process_metrics_summary['sensory_captures']['quantum_channel'] = quantum_channel_sensory
...
#         # INTEGRATION POINT 3: After _transfer_creator_aspects() call
#         # Create glyph for aspect transfer
#         transfer_glyph_id, transfer_encoded_path = create_aspect_transfer_glyph(
#             soul_spark, transfer_metrics
#         )
            
#         # Capture sensory data for aspect transfer
#         aspect_transfer_sensory = capture_aspect_transfer_sensory_data(
#             soul_spark, transfer_metrics, transfer_glyph_id, transfer_encoded_path
#         )
            
#         process_metrics_summary['sensory_captures']['aspect_transfer'] = aspect_transfer_sensory
...
#         # INTEGRATION POINT 4: After _form_resonance_patterns() call
#         # Create glyph for resonance completion
#         completion_glyph_id, completion_encoded_path = create_resonance_completion_glyph(
#             soul_spark, pattern_metrics
#         )
            
#         # Capture sensory data for resonance completion
#         resonance_completion_sensory = capture_resonance_completion_sensory_data(
#             soul_spark, pattern_metrics, completion_glyph_id, completion_encoded_path
#         )
            
#         process_metrics_summary['sensory_captures']['resonance_completion'] = resonance_completion_sensory
...
#         # INTEGRATION POINT 5: Before final return statement
#         # Save all sensory data for soul echos processing
#         if 'sensory_captures' in process_metrics_summary and len(process_metrics_summary['sensory_captures']) > 0:
#             try:
#                 sensory_data_file = save_creator_entanglement_sensory_data(
#                     soul_spark, 
#                     process_metrics_summary['sensory_captures']
#                 )
#                 process_metrics_summary['sensory_data_file'] = sensory_data_file
#                 logger.info(f"Creator entanglement sensory data saved for soul echos processing: {sensory_data_file}")
#             except Exception as e:
#                 logger.warning(f"Failed to save sensory data for soul echos: {e}")
        
#         # Calculate final process metrics
#         end_time_dt = datetime.now()
#         total_duration = (end_time_dt - start_time_dt).total_seconds()
            
#         process_metrics_summary.update({
#             'process_name': 'creator_entanglement',
#             'soul_id': soul_spark.spark_id,
...
#             'success': True,
#             'sensory_captures_count': len(process_metrics_summary.get('sensory_captures', {})),
#             'glyphs_created': [
#                 capture.get('glyph_id') for capture in process_metrics_summary.get('sensory_captures', {}).values() 
#                 if capture.get('glyph_id')
#             ],
#             'encoded_glyphs': [
#                 capture.get('encoded_glyph_path') for capture in process_metrics_summary.get('sensory_captures', {}).values()
#                 if capture.get('encoded_glyph_path')
#             ]
#         })
            
#         logger.info(f"--- Creator Entanglement Complete for Soul {spark_id} ---")
#         logger.info(f"Duration: {total_duration:.2f}s, Sensory Captures: {len(process_metrics_summary.get('sensory_captures', {}))}, Glyphs: {len([g for g in process_metrics_summary.get('glyphs_created', []) if g])}")
...
#             'steps': process_metrics_summary.get('steps', {}),
#             'sensory_captures': process_metrics_summary.get('sensory_captures', {}),  # ADD THIS LINE
#             'glyphs_created': [
#                 capture.get('glyph_id') for capture in process_metrics_summary.get('sensory_captures', {}).values() 
#                 if capture.get('glyph_id')
#             ],
...
# def save_experience_to_soul_echoes(spark_id: str, experience_data: dict):
#     """Save the captured experience data as a soul echo to be converted to a memory fragment when mycelial network forms."""
