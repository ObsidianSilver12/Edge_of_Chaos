...
EDGE_OF_CHAOS_TARGET = 0.618  # Critical value for emergence (1/φ)
MIN_COMPLEXITY = 0.2
MAX_COMPLEXITY = 0.9
...
class EdgeOfChaos:
    ...
    def calculate_current_state(self):
        ...
        edge_proximity = 1.0 - abs(self.chaos_level - EDGE_OF_CHAOS_TARGET)
    ...
    def adjust_toward_edge(self, intensity=0.5):
        ...
        distance_from_edge = abs(self.chaos_level - EDGE_OF_CHAOS_TARGET)
        
        # Determine direction of adjustment
        if self.chaos_level > EDGE_OF_CHAOS_TARGET:
    ...
    def induce_phase_transition(self, target_complexity=0.8, max_attempts=5):
        ...
        Returns:
            bool: Success status
        """
        logger.info(f"Attempting to induce phase transition to complexity {target_complexity}")
        ...
        for attempt in range(max_attempts):
            ...
            required_chaos = EDGE_OF_CHAOS_TARGET
            ...
            if self.complexity >= target_complexity:
                logger.info(f"Phase transition successful after {attempt + 1} attempts")
        ...
        logger.warning(f"Failed to induce phase transition after {max_attempts} attempts")
    ...
    def get_complexity_metrics(self):
        ...
        edge_distance = abs(self.chaos_level - EDGE_OF_CHAOS_TARGET)
